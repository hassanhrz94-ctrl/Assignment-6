...
const ProductCard = ({ product, carts, setCarts }) => {
    ...
    const handleBtn = () => {
        const isExist = carts.find(item => item.name === product.name);
    ...
    return (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 flex flex-col gap-4 w-full">
            ...
            <button
                onClick={handleBtn}
                className="w-full py-3 rounded-xl text-white font-semibold text-sm transition-all duration-200 mt-auto bg-purple-600 hover:bg-purple-700 active:scale-95"
            >
                Buy Now
            </button>
        </div>
